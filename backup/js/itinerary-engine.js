 ...
 * filters = { tripDuration, budgetTier, tripType, interests[], preferredRegion }
 * Returns scored array of matching activities (sorted by relevance score).
 */
function filterActivities(activities, filters) {
  const { budgetTier = "mid", tripType = "solo", interests = [], preferredRegion = "none" } = filters;
...
function buildDailyItinerary(filteredActivities, tripDuration, preferredRegion) {
  ...
  const transport = filteredActivities.find(a => a.category.indexOf("Transport") === 0);
  ...
  // Budget summary: approximate daily activity spend (activities + avg transport share)
  const avgCostPerDay = totalActivityCount > 0
    ? Math.round(totalActivityCost / tripDuration + 30) // +30 transport/food/activity fees
    : 0;

  return { days, estimatedActivityCostPerDay: avgCostPerDay, warnings };
}
...
window.BaliEngine = { filterActivities, buildDailyItinerary, generateInsiderTips, travelTime };
