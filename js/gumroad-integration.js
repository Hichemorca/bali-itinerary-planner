...
(function () {
  ...
  window.Gumroad.buy = function (product, purchase) {
    ...
    if (window.trackFunnel) window.trackFunnel.purchased();
...
/**
 * Track Buy button click (call from any CTA that triggers Gumroad checkout)
 */
function trackBuyClick() {
  if (window.trackFunnel) window.trackFunnel.buyClick();
}
